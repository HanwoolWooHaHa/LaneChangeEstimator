...
public:
	...
	enum{ MEASUREMENT = 0, FILE, LANE, FEATURE, ESTIMATION_RESULT, NORTHEAST, SOUTHWEST };
	...
	*/
	...
	double GetData( int type, int time, int index )
	{
		switch( type )
		{
		case MEASUREMENT:
	...
	double GetData(int nType, int nLane, int nIndex, int nColumn)
	{
		if (nType == LANE)
			return m_dLaneData[nLane][nIndex][nColumn];
		else
			return -1.0;
	}
