...
void CBirdView::paintGL()
{
	...
	drawingPredictionArea(dGlobalPosX, dGlobalPosY, dAttitude);
	drawLine();
	//drawLineMarkings();
...
void CBirdView::drawingPredictionArea(double dGlobalOwnX, double dGlobalOwnY, double dAttitude)
{
	//! 自車が走っている現車線
	...
	glVertex3f(0.0, -6.0, 0.2);
	glVertex3f(50.0, -6.0, 0.2);
	glVertex3f(50.0, -2.0, 0.2);
...
void CBirdView::drawVehicle( int type, int index )
{
	double dGlobalX = 0.0;
	double dGlobalY = 0.0;
...
void CBirdView::renderVehicle(double dPosX, double dPosY, double dAttitude, const QColor& color)
{
	glPushMatrix();
